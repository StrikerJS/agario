...
class PhantomServer {
	...
	loadDriver() {
		...
		this.driver.then(page => {
			...
			//console.log(`MEGABOTS SERVER LOADED PAGE: ${this.agarioURL} IN ${Date.now() - beforeLoad} MILLISECONDS!.`.yellow);
			this.onAgarioLoaded();
		});
	}

	setPageProperties() {
		this.page.on('viewportSize', {
			width: 800,
			height: 600
		});

		this.page.on('onResourceRequested', function (requestData, networkRequest) {
			//if (!/(agar\.io|miniclip)/g.test(requestData.url)) console.log(networkRequest);
		});
	}
